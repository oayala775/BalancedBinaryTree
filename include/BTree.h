...
class BTree {
 ...
 public:
  ...
  int balanceFactor(Position& pos);
  void simpleRightRotation(Position& pos);
  void simpleLeftRotation(Position& pos);
  void doubleRightRotation(Position& pos);
  void doubleLeftRotation(Position& pos);

  void balance(Position& pos);
...
template <typename T>
void BTree<T>::balance(Position& pos) {
  if (balanceFactor(pos) < -1) {
    if (balanceFactor(pos->getLeft()) == -1)
      simpleRightRotation(pos);
    else
      doubleRightRotation(pos);
    return;
  }

  if (balanceFactor(pos) > 1) {
    if (balanceFactor(pos->getRight()) == 1)
      simpleLeftRotation(pos);
    else
      doubleLeftRotation(pos);
    return;
  }
}

template <typename T>
int BTree<T>::balanceFactor(Position& pos) {
  return (pos == nullptr)
             ? 0
             : getHeight(pos->getRight()) - getHeight(pos->getLeft());
}

template <typename T>
void BTree<T>::simpleRightRotation(Position& pos) {
  Position auxLeft(pos->getLeft());
  Position auxRight(pos->getLeft()->getRight());

  pos->setLeft(auxRight);
  auxLeft->setRigth(pos);
  pos = auxLeft;
}

template <typename T>
void BTree<T>::simpleLeftRotation(Position& pos) {
  Position auxRight(pos->getRight());
  Position auxLeft(pos->getRight()->getLeft());

  pos->setRigth(auxLeft);
  auxRight->setLeft(pos);
  pos = auxRight;
}

template <typename T>
void BTree<T>::doubleRightRotation(Position& pos) {
  Position aux1(pos->getLeft());
  Position aux2(aux1->getRight());
  Position aux3(aux2->getLeft());
  Position aux4(aux2->getRight());
...
template <typename T>
void BTree<T>::doubleLeftRotation(Position& pos) {
  Position aux1(pos->getRight());
  Position aux2(aux1->getLeft());
  Position aux3(aux2->getRight());
  Position aux4(aux2->getLeft());
